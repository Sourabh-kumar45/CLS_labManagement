...
const TeacherNavbar = () => {
  ...
  const { teacherid } = useParams(); // Get user ID from URL parameters

  return (
    <nav className="bg-zinc-100 shadow-lg mb-6 fixed top-0 left-0 w-full z-50">
  <div className="max-w-7xl mx-auto px-4">
    <div className="flex justify-between items-center py-4">
      {/* Logo */}
      <div className="text-2xl font-bold text-blue-600 hover:cursor-pointer">
        <a href={teacherid ? `/teacher/${teacherid}` : '/'}>CLS</a>
      </div>

      {/* Desktop Links */}
      <div className="hidden md:flex space-x-6">
          <>
            {/* <a href={`/student/${teacherid}/itemIssueDepartmentList`} className="text-gray-700 hover:text-blue-600">Department</a>
            <a href={`/student/${teacherid}/stuAbt`} className="text-gray-700 hover:text-blue-600">Issued Items</a>
            <a href={`/student/${teacherid}/achievement`} className="text-gray-700 hover:text-blue-600">Achievements</a>
            <a href={`/student/${teacherid}/help`} className="text-gray-700 hover:text-blue-600">Help</a> */}
            <a
              href="/"
